...
struct DocSearcherApp {
    ...
    pending_open: Option<SearchResult>,
    pending_copy: Option<PathBuf>,
}
...
impl DocSearcherApp {
    fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        Self {
            ...
            selected_hit: None,
            show_settings: false,
            temp_index_dir: config.index_dir.clone(),
            temp_pdf_reader: config.pdf_reader.clone().unwrap_or_default(),
            temp_ignore_case: config.ignore_case,
            show_help: false,
    ...
    fn export_results_to_csv(&self, path: &Path) -> Result<()> {
        ...
        for res in &self.results {
            let file_size_str = if let Ok(Some(meta_bytes)) = self.meta_db.get(&res.file_id.to_le_bytes()) {
                if let Ok(meta) = bincode::deserialize::<DocMeta>(&meta_bytes) {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        for msg in progress_msgs {
            match msg {
                ...
                IndexMsg::Done => {
                    ...
                    self.config.last_index_time = Some(now);
                    self.save_config();
                }
            }
        }
        ...
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_source("view_combo")
                    .selected_text(match self.config.result_view {
                        ResultViewMode::Compact => "📋 紧凑",
                        ResultViewMode::Detailed => "📊 详细",
                    })
                    .show_ui(ui, |ui| {
                ...
                if ui.button("⚙️ 设置").clicked() {
                    ...
                    self.temp_ignore_case = self.config.ignore_case;
                    self.font_scale_tmp = self.config.font_scale;
                }
        ...
        if self.index_state != IndexState::Idle && self.progress_total > 0 {
            egui::TopBottomPanel::top("progress_panel").show(ctx, |ui| {
                let progress = self.progress_current as f32 / self.progress_total.max(1) as f32;
                ui.add(egui::ProgressBar::new(progress).text(format!("索引进度: {}/{}", self.progress_current, self.progress_total)));
            });
        }
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui.checkbox(&mut ignore, "忽略大小写").clicked() {
                    ...
                    self.save_config();
                    self.log("大小写设置已更改，建议重建索引以完全生效".to_string());
                }
                ...
                if self.show_query_tips {
                    ui.label("支持通配符 *, ?, 布尔 AND/OR/NOT, 短语 \"...\"");
                }
            });
            ...
            ui.label(format!(
                "已索引文件: {} | 上次索引: {} | {}",
                self.total_indexed,
                self.config.last_index_time.as_deref().unwrap_or("无"),
                self.index_status
            ));
            ...
            if !self.results.is_empty() {
                if ui.button("📤 导出结果CSV").clicked() {
                    if let Some(path) = FileDialog::new()
                        .set_file_name("search_results.csv")
                        .save_file()
                    {
                        match self.export_results_to_csv(&path) {
                            Ok(()) => self.log(format!("结果已导出到 {}", path.display())),
            ...
            ui.columns(2, |columns| {
                ...
                ScrollArea::vertical().show(left, |ui| {
                    let item_height = match self.config.result_view {
                                ...
                                match self.config.result_view {
                                    ...
                                    ResultViewMode::Detailed => {
                                        ...
                                        ui.label(format!("修改: {} | 大小: {}",
                                            format_timestamp(res.mtime),
                                            self.meta_db.get(&res.file_id.to_le_bytes())
                                                .ok()
                                                .and_then(|v| v.map(|b| bincode::deserialize::<DocMeta>(&b).ok()).flatten())
        ...
        // 帮助窗口
        if self.show_help {
            egui::Window::new("帮助与关于")
                .collapsible(false)
                .show(ctx, |ui| {
                    ui.label("本地文档检索系统 v1.0");
                    ui.separator();
                    ui.label("快捷键：");
                    ui.label("Ctrl+F 聚焦搜索框， Esc 清除搜索");
                    ui.label("双击结果打开文件");
                    ui.separator();
                    ui.label("查询语法：");
                    ui.label("支持通配符 * ? ，布尔运算 AND/OR/NOT，短语 \"...\"");
                    ui.label("索引支持 PDF, DOCX, TXT 格式");
