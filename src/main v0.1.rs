...
struct DocSearcherApp {
    ...
    root_dir: Option<PathBuf>,          // 始终为绝对路径
...
impl DocSearcherApp {
    ...
    fn open_with_command(&mut self, file_path: &Path, page: u64, keyword: &str) {
        ...
        let abs_path = if file_path.is_absolute() {
            ...
            let base = match &self.root_dir {
                Some(dir) => dir.clone(),
                None => {
                    self.log("错误：检索目录未设置！".into());
                    return;
                }
            };
        ...
        // 调试日志：显示最终路径
        self.log(format!("[调试] 传递给阅读器的绝对路径: {}", abs_path.display()));
            ...
            ui.columns(2, |columns| {
                ...
                if let Some(idx) = self.selected_result {
                    if let Some(res) = self.results.get(idx) {
                        ...
                        ScrollArea::vertical().show(right, |ui| {
...
fn full_scan_and_index(
    ...
    for entry in WalkDir::new(root).follow_links(true).into_iter().filter_map(|e| e.ok()) {
        ...
        // entry.path() 已经是绝对路径，因为 root 是绝对路径
